...
def main():
    ...
    arg('--data-dir', type=str, default="/home/yjheo/Deepfake/dataset/dfdc_facebook/train/")
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        if args.local_rank == 0:
            ...
            if (epoch + 1) % args.test_every == 0:
                bce_best = evaluate_val(args, val_data_loader, bce_best, model, conf,
...
def evaluate_val(args, data_val, bce_best, model, conf, snapshot_name, current_epoch, summary_writer):
    print("Test phase")
    model = model.eval()

    bce, probs, targets = validate(model, conf, data_loader=data_val)
    if args.local_rank == 0:
...
def validate(net, conf, data_loader, prefix=""):
    ...
    with torch.no_grad():
        for sample in tqdm(data_loader):
            ...
            if len(imgs[0]) > 45 :
                ...
                temp = imgs[0][-3:].repeat(15 - len(imgs[0])//3, 1, 1).view(1,-1,384,384)
                imgs = torch.cat([imgs, temp], dim=1) #42
            '''
            if conf['network'] == 'DeepFakeClassifier_Distill':
                _, out, _ = net(imgs) #distill 0524 # video : imgs[:,::2, :,:] # eye
            else:
                out = net(imgs)
...
def train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
    ...
    for i, sample in pbar:
        ...
        if only_valid:
            ...
            if conf['network'] == 'DeepFakeClassifier_Distill':
                student_logits, distill_logits, teacher_logits = student_logits[valid_idx], distill_logits[valid_idx], teacher_logits[valid_idx] #distill 0524
            else:
        ...
        fake_idx = labels > 0.5
        real_idx = labels <= 0.5
        img_fake_index = torch.where(fake_idx)
        img_real_index = torch.where(real_idx)
        alpha = 0.5
        ohem = conf.get("ohem_samples", None)
        if torch.sum(fake_idx * 1) > 0:
            if conf['network'] == 'DeepFakeClassifier_Distill':
                fake_student_loss = loss_functions["classifier_loss"](student_logits[fake_idx], labels[fake_idx])
                fake_distill_loss = loss_functions["classifier_loss"](distill_logits[fake_idx], torch.sigmoid(teacher_logits[fake_idx]))
                fake_loss = fake_student_loss * alpha + fake_distill_loss * (1-alpha)
            else:
                fake_loss =loss_functions["classifier_loss"](out_labels[fake_idx], labels[fake_idx])
        if torch.sum(real_idx * 1) > 0:
            if conf['network'] == 'DeepFakeClassifier_Distill':
                real_student_loss = loss_functions["classifier_loss"](student_logits[real_idx], labels[real_idx])
                real_distill_loss = loss_functions["classifier_loss"](distill_logits[real_idx], torch.sigmoid(teacher_logits[real_idx]))
                real_loss = real_student_loss * alpha + real_distill_loss * (1 - alpha)
            else:
        ...
        #image
        #loss = fake_loss if real_loss ==0 else real_loss
